...
from unit_tests.test_cases import Responses, custom_checking
...
if st.button("Run Tests"):
    ...
    results_df = pd.DataFrame(
        columns=["Test ID", "Question", "Output", "Sources", "Result", "Missing"]
    )
    rows_to_add = []
    ...
    for data in data_list:
        ...
        missing = (
            ", ".join(data[4]) if len(data) > 4 and not test_cases[test_id] else ""
        )
        rows_to_add.append(
            {
                "Test ID": test_id,
                ...
                "Result": result,
                "Missing": ", ".join(data[4])
                if len(data) > 4 and not test_cases[test_id]
                else "",
            }
        )
