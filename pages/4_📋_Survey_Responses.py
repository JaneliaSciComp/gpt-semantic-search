...
def fetch_all_survey_responses(client, class_name="SurveyResponses", limit=100):
    ...
    while True:
        ...
        response = client.data_object.get(
            class_name=class_name, limit=limit, offset=offset
        )
...
def display_objects(objects):
    if st.session_state["admin_toggle"] == True:
        for index, obj in enumerate(objects, start=1):
            ...
            st.markdown(
                f"<small>Creation Time: {obj['creationTimeUnix']}</small>",
                unsafe_allow_html=True,
            )
            st.markdown(
                f"<small>Last Update Time: {obj['lastUpdateTimeUnix']}</small>",
                unsafe_allow_html=True,
            )

            st.markdown("##### Properties", unsafe_allow_html=True)
            st.markdown(
                f"<h3 style='margin-bottom:0;'>Query:</h3><p style='font-size:20px; color:#FFC0CB;'>{obj['properties']['query']}</p>",
                unsafe_allow_html=True,
            )
            st.markdown(
                f"<h3 style='margin-bottom:0;'>Response:</h3><p style='font-size:20px; color:#FFC0CB;'>{obj['properties']['response']}</p>",
                unsafe_allow_html=True,
            )
            ...
            selected_survey_value = st.selectbox(
                "Survey",
                options=survey_options,
                index=survey_options.index(current_survey_value),
                key=f"survey_{index}",
            )
