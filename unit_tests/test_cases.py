...
def custom_checking(Responses):
    ...
    for response in Responses:
        ...
        if current_id == "00a4917b-e25f-43da-9771-bc4cf742c3ec":
            ...
            if "https://wikis.janelia.org/display/SCSW/Getting+Help" not in actual_sources:
            ...
            missing = ["11:44 slack message in #storage in sources"] if not test_cases[current_id] else []
            ...
            missing = []
            if "yuz10@janelia.hhmi.org" not in actual_output and "cryoemfacility@janelia.hhmi.org" not in actual_output:
                missing.append("either yuz10@janelia.hhmi.org or cryoemfacility@janelia.hhmi.org in output")
            ...
            missing = ["JENNIFER LIPPINCOTT-SCHWARTZ in output"] if not test_cases[current_id] else []
        elif current_id == "3afec7cb-9a93-478f-ae0b-fa1815f57395":
            test_cases[current_id] = any(num in actual_output for num in ["21", "20", "9", "7"])
            missing = ["either '21', '20', '9', or '7' in output"] if not test_cases[current_id] else []
        ...
        data_list.append([input_question, actual_output, actual_sources, current_id, missing])
        print()
