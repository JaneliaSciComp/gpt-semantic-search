...
Responses = [
    ...
    },
    {
        ...
        "Context": [
            "MDAS stands for 'microscope direct attached storage.' It is a storage system designed to handle high-speed data capture directly from microscopes.",
    ...
    },
    {
        ...
        "Source Notes": "NA",
        "Context": [
            "If you're new at Janelia, you might want to explore and join various Slack channels to get integrated into the community.",
    ...
    },
    {
        ...
        "Source Notes": "NA",
        "Context": [
            "Mark Kittisopikul and Ben Arthur appear to have significant knowledge about Julia at Janelia.",
    ...
    },
    {
        ...
        "Context": [
            "NeuronBridge is a project developed at Janelia that provides a platform for managing and searching neuron imagery.",
    ...
    },
    {
        ...
        "Source Notes": "N/A",
        "Context": [
            "There are 7 members in the Misha Arhens lab team, 2 visiting scientists in residence, 11 other science contributors, and 1 lab coordinator"
        ],
    },
]
...
def custom_checking(Responses):
    ...
    print(f"\n{Fore.CYAN}{Style.BRIGHT}Running Custom Tests{Style.RESET_ALL}\n")
    ...
    for response in Responses:
        ...
        print(f"{Fore.YELLOW}Test ID: {current_id}")
        print(f"Question: {input_question}")
        print(f"Output: {actual_output[:100]}...")  # Truncate for readability
        print(f"Sources: {actual_sources[:100]}...")  # Truncate for readability

        if current_id == "00a4917b-e25f-43da-9771-bc4cf742c3ec":
            test_cases[current_id] = (
                "support@hhmi.org" in actual_output
                and "https://wikis.janelia.org/display/SCSW/Getting+Help"
                in actual_sources
            )
            missing = []
            if "support@hhmi.org" not in actual_output:
                missing.append("support@hhmi.org in output")
            if (
                "https://wikis.janelia.org/display/SCSW/Getting+Help"
                not in actual_sources
            ):
                missing.append("Getting+Help wiki link in sources")
        elif current_id == "024cf273-1920-43cb-a7e8-c3191af81c59":
            test_cases[current_id] = (
                "help desk" in actual_output
                and "support@hhmi.org" in actual_output
                and "https://wikis.janelia.org/display/SCSW/Getting+Help"
                in actual_sources
            )
            missing = []
            if "help desk" not in actual_output:
                missing.append("'help desk' in output")
            if "support@hhmi.org" not in actual_output:
                missing.append("support@hhmi.org in output")
            if (
                "https://wikis.janelia.org/display/SCSW/Getting+Help"
            ...
            missing = (
                ["11:44 slack message in #storage in sources"]
                if not test_cases[current_id]
                else []
            )
        elif current_id == "0d0fad42-ba6a-4223-8bd2-2e58554d51cb":
            test_cases[current_id] = "python" in actual_output
            missing = ["'python' in output"] if not test_cases[current_id] else []
        elif current_id == "189988ac-433e-4640-9684-5c46f40e54b5":
            test_cases[current_id] = (
                "Mark Kittisopikul" in actual_output and "Ben Arthur" in actual_output
            )
            missing = []
            if "Mark Kittisopikul" not in actual_output:
                missing.append("Mark Kittisopikul in output")
            if "Ben Arthur" not in actual_output:
                missing.append("Ben Arthur in output")
        elif current_id == "2fa07b48-9130-458d-a38f-ae4860a901e3":
            test_cases[current_id] = (
                "yuz10@janelia.hhmi.org" in actual_output
                or "cryoemfacility@janelia.hhmi.org" in actual_output
            ) and "Cryo-Electron Microscopy" in actual_sources
            missing = []
            if (
                "yuz10@janelia.hhmi.org" not in actual_output
                and "cryoemfacility@janelia.hhmi.org" not in actual_output
            ):
                missing.append(
                    "either yuz10@janelia.hhmi.org or cryoemfacility@janelia.hhmi.org in output"
                )
            if "Cryo-Electron Microscopy" not in actual_sources:
                missing.append("Cryo-Electron Microscopy in sources")
        elif current_id == "38af623d-35d3-48ab-8587-a40aa8d2694b":
            test_cases[current_id] = "JENNIFER LIPPINCOTT-SCHWARTZ" in actual_output
            missing = (
                ["JENNIFER LIPPINCOTT-SCHWARTZ in output"]
                if not test_cases[current_id]
                else []
            )
        elif current_id == "3afec7cb-9a93-478f-ae0b-fa1815f57395":
            test_cases[current_id] = any(
                num in actual_output for num in ["21", "20", "9", "7"]
            )
            missing = (
                ["either '21', '20', '9', or '7' in output"]
                if not test_cases[current_id]
                else []
            )

        result = "PASSED" if test_cases[current_id] else "FAILED"
        color = Fore.GREEN if result == "PASSED" else Fore.RED
        print(f"{color}Result: {result}{Style.RESET_ALL}")
        if result == "FAILED":
            print(f"{Fore.RED}Missing: {', '.join(missing)}{Style.RESET_ALL}")

        data_list.append(
            [input_question, actual_output, actual_sources, current_id, missing]
        )
        print()
...
# Remove the print_test_results function as it's now integrated into custom_checking
