    ...
    def _init_clients(self):
        ...
        try:
            ...
            if slack_token:
                ...
                if not res["ok"]:
                    console.print(
                        f"[yellow]Warning: Slack API error: {res['error']}[/yellow]"
                    )
                    self.slack_client = None
            else:
                console.print(
                    "[yellow]Warning: SLACK_TOKEN not found. Slack links will not be available.[/yellow]"
                )
...
def main():
    ...
    parser.add_argument(
        "-q", "--query", type=str, help="Search query (non-interactive mode)"
    )
    parser.add_argument(
        "-m", "--model", type=str, default="gpt-4o", help="OpenAI model to use"
    )
    parser.add_argument(
        "-t", "--temperature", type=float, default=0.0, help="Response temperature"
    )
    parser.add_argument(
        "-a",
        "--alpha",
        type=float,
        default=0.55,
        help="Search alpha (0=semantic, 1=keyword)",
    )
    parser.add_argument(
        "-r", "--results", type=int, default=3, help="Number of results to return"
    )
    parser.add_argument(
        "--hyde", action="store_true", help="Enable HyDE query transformation"
    )
    parser.add_argument(
        "--class-prefix", type=str, default="Janelia", help="Weaviate class prefix"
    )
