...
def setup_logging() -> logging.Logger:
    ...
    log_file = os.path.join(
        "logs", f"slack_indexer_{datetime.now().strftime('%Y%m%d')}.log"
    )
...
def find_folders_to_process(
    ...
    logger.info(
        f"Latest database timestamp: {latest_db_timestamp} "
            ...
            if folder.startswith("success_run_"):
                try:
                    ...
                    if timestamp > cutoff_timestamp:
                        ...
                        logger.info(
                            f"Will index: {folder} ({datetime.fromtimestamp(timestamp)})"
                        )
                except ValueError:
                    continue
...
def mark_folder_status(
    workspace_name: str, folder_name: str, status: str, logger: logging.Logger
) -> bool:
...
class SlackLoader:
    ...
    def parse_message(self, message):
        ...
        text_msg = re.sub(
            r"<@([^>]+)>", lambda m: self.id2realname.get(m.group(1), m.group(1)), text
        )
    ...
    def load_documents(self, channel_name: str) -> List[Document]:
        ...
        if doc_text:
            doc = Document(
                text=doc_text,
                extra_info={"source": SOURCE, "scraped_at": datetime.now().timestamp()},
            )
            documents.append(doc)
...
def process_folder(
    ...
    try:
        ...
        for i, doc in enumerate(documents, 1):
            ...
            logger.info(
                f"Indexing document {i}/{len(documents)}: '{text_preview}...' (source: {doc.extra_info.get('source', 'Unknown')})"
            )
...
def retry_failed_folders(
    ...
    for workspace_name, folder_name, timestamp in failed_folders:
        ...
        success = process_folder(
            workspace_name, processing_folder, weaviate_url, class_prefix, debug
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Slack Incremental Indexer - smart processing of scraped data"
    )
    parser.add_argument(
        "-i", "--input", type=str, help="Path to specific folder to process"
    )
    parser.add_argument(
        "-w",
        "--weaviate-url",
    ...
    parser.add_argument(
        ...
        type=str,
        default="Janelia",
        help="Class prefix in Weaviate",
    )
    parser.add_argument(
        "-d", "--debug", action="store_true", help="Enable debug logging"
    )
    parser.add_argument(
        "--retry-only", action="store_true", help="Only retry failed folders"
    )
    parser.add_argument(
        "--buffer",
        type=float,
        default=0.5,
        help="Buffer time in hours for indexing cutoff",
    )
    parser.add_argument(
        "--data-path",
        type=str,
        default="../../data/slack",
        help="Path to slack data directory",
    )
    ...
    retry_stats = retry_failed_folders(
        args.data_path, args.weaviate_url, args.class_prefix, args.debug
    )

    if retry_stats["attempted"] > 0:
        logger.info(
            f"Retry summary: {retry_stats['succeeded']} succeeded, {retry_stats['failed']} failed out of {retry_stats['attempted']} attempted"
        )
    ...
    if args.input:
        ...
        folders_to_process = find_folders_to_process(
            args.data_path, indexer, args.buffer
        )
    ...
    for workspace_name, folder_name, timestamp in folders_to_process:
        logger.info(
            f"Processing folder: {folder_name} ({datetime.fromtimestamp(timestamp) if timestamp > 0 else 'manual'})"
        )
