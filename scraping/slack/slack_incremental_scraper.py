...
def setup_logging() -> logging.Logger:
    ...
    log_file = os.path.join(
        "logs", f"slack_scraper_{datetime.now().strftime('%Y%m%d')}.log"
    )
        ...
        try:
            ...
            for message in messages:
                ...
                if reply_count > 0 and msg_ts not in threaded_messages:
                    thread_messages = fetch_complete_thread(
                        client, channel_id, msg_ts, logger
                    )
...
def enrich_messages_with_user_profiles(
    ...
    for user in users:
        if user.get("id") and not user.get("deleted", False):
            user_id = user["id"]
            profile = {
                "real_name": user.get("profile", {}).get(
...
def save_messages(
    ...
    for date_str, date_messages in messages_by_date.items():
        base_dir = os.path.join(
            data_base_path, workspace_name, f"run_{run_timestamp}", channel_name
        )
        ...
        new_messages = [
            msg for msg in date_messages if msg.get("ts") not in existing_ts
        ]
...
def save_metadata_files(
    ...
    for channel in channels:
        if channel.get("id") and channel.get("name"):
            channel_entry = {"id": channel["id"], "name": channel["name"]}
...
def mark_run_status(
    workspace_name: str, run_timestamp: int, status: str, logger: logging.Logger
) -> bool:
...
def cleanup_failed_run(
    workspace_name: str, run_timestamp: int, logger: logging.Logger
) -> None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Slack Incremental Scraper - automatically continues from last successful run"
    )
    parser.add_argument(
        "--start-from",
        type=float,
    ...
    parser.add_argument(
        "--end-at", type=float, help="Unix timestamp to end scraping at (default: now)"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.start_from:
            ...
            logger.info(
                f"Using custom start timestamp: {datetime.fromtimestamp(start_timestamp)}"
            )
        else:
            ...
            if latest_timestamp:
                ...
                logger.info(
                    f"Auto-continuing from last successful run: {datetime.fromtimestamp(latest_timestamp)}"
                )
            else:
                # No previous runs, start from 24 hours ago
                start_timestamp = (datetime.now() - timedelta(days=1)).timestamp()
                logger.info(
                    f"No previous runs found, starting from 24 hours ago: {datetime.fromtimestamp(start_timestamp)}"
                )
        ...
        logger.info(
            f"Scraping window: {datetime.fromtimestamp(oldest_ts)} to {datetime.fromtimestamp(newest_ts)}"
        )
