...
def update_message(client, channel, timestamp, text, blocks=None):
    client.chat_update(channel=channel, ts=timestamp, text=text, blocks=blocks)


def generate_response_with_animation(text, channel, thread_ts, client):
    # Send an initial message with a random thinking phrase
    thinking_phrases = ["Thinking...", "Processing your request...", "Hmmmmm..."]
    ...
    def animate():
        ...
        while not stop_event.is_set():
            update_message(
                client, channel, message_ts, f"{loading_chars[i]} {initial_message}"
            )
...
def process_message(event, client):
    ...
    Thread(
        target=generate_response_with_animation, args=(text, channel, thread_ts, client)
    ).start()
